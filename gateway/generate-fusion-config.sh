...
# Check if reviews service is available (macOS compatible)  
echo "Checking reviews service availability..."
for i in {1..30}; do
  if curl -s http://localhost:4002/graphql > /dev/null 2>&1; then
...
cat > .fusion/subgraphs/accounts/subgraph-config.json << 'EOF'
{
  "subgraph": "accounts",
  "http": {
    "baseAddress": "http://localhost:4001/graphql"
  }
}
EOF
...
cat > .fusion/subgraphs/reviews/subgraph-config.json << 'EOF'
{
  "subgraph": "reviews", 
  "http": {
    "baseAddress": "http://localhost:4002/graphql"
  }
}
EOF
...
echo "Fusion configuration generated successfully!"